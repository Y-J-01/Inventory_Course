...
FVector2D UInv_WidgetUtils::GetClampedWidgetPosition(const FVector2D& Boundary, const FVector2D& WidgetSize,
	const FVector2D& MousePos)
{
	FVector2D ClamepdPosition = MousePos;
	
	if (MousePos.X + WidgetSize.X > Boundary.X)
	{
		ClamepdPosition.X = Boundary.X - WidgetSize.X;
	}
	if (MousePos.X < 0.f)
	{
		ClamepdPosition.X = 0.f;
	}
	
	if (MousePos.Y+ WidgetSize.Y > Boundary.Y)
	{
		ClamepdPosition.Y = Boundary.Y - WidgetSize.Y;
	}
	if (MousePos.Y < 0.f)
	{
		ClamepdPosition.Y = 0.f;
	}
	
	
	
	return ClamepdPosition;
}
